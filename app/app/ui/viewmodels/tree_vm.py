...
class TreeItem:
    def __init__(self, task: Task, parent: Optional["TreeItem"] = None):
        self.task = task
        self.parent = parent
...
class TaskTreeModel(QAbstractItemModel):
    ...
    def reload(self):
        ...
        def build(parent_item: Optional[TreeItem], parent_id: Optional[int]):
            for t in self.repo.children(parent_id):
                node = TreeItem(t, parent_item)
                if parent_item:
    ...
    def setData(self, index, value, role=Qt.EditRole):
        ...
        upd = UpdateTask(self.repo, None)
        upd.execute(UpdateTaskInput(item.task.id, {"title": str(value)}))
        item.task.title = str(value)
